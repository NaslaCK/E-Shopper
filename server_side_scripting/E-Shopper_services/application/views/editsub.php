...
function edit(subclick)
{
	$.ajax({
		'type' :"POST",
		'url' :"<?php echo base_url(); ?>"+"index.php/welcome/editsub",
		'datatype':"json",
		'data':{name:subclick },
		'success':function(data){
			...
			for (var i = 0 ; i <=abc.length-1; i++) {
				...
				toAppend+="<input type='hidden' name='did' value='"+abc[i].pk_int_sub_id+"'><input type='text' id='txt1' name='subcat'  id='inputcategory' class='form-control' placeholder='"+abc[i].vchr_sub_name+"'><br><button type='submit' name='sub' class='btn btn-danger'>"+'Update'+"</button>";
			};
			$('#id1').append(toAppend);
		}
	});
