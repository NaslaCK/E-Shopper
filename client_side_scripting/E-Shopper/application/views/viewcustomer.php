...
<head>
    ...
    <title>View Customers | E-Shopper</title>
    	...
    	function purchasedetails(id)
{
	$.ajax({
		'type' :"POST",
		'url' :"http://localhost/E-Shopper_services/index.php/welcome/detailofproducts",
		'datatype':"json",
		'data':{name:id},
		'success':function(data){
			...
			var toAppend="<tr><th>SI NO</th><th>Product Name</th><th>Price</th><th>Quantity</th><th>Total Amount</th><th>Date of Purchase</th></tr>";
			for (var i = 0 ; i <=abc.length-1; i++) {


				toAppend+="<tr><td>"+abc[i].pk_int_purchase_id+"</td><td>"+abc[i].vchr_product_name+" </td><td>"+abc[i].int_price+"</td><td>"+abc[i].int_quantity+"</td><td>"+abc[i].int_total_amount+"</td><td>"+abc[i].dat_date+"</td></tr> ";
			};
			$('#det1').append(toAppend);
		}
	});
}
...
<body>
	...
	<section>
    	...
    	<div class="bg">
	    	<div class="row">    		
	    		<div class="col-sm-12">    			   			
					<h2 class="title text-center">View <strong>Customers</strong></h2>    			    				    				
					
				</div>			 		
			</div>
			<div class="row">  	
	    		<div class="col-sm-12">
					<div class="col-sm-offset-1 col-sm-10" id="div1">
						...
						<?php
							foreach ($viewcust as $row) 
							{
								echo"<tr>";
								// echo "<td>".$row->pk_int_reg_id."</td><td>".$row->vchr_fname." </td><td>".$row->vchr_lname."</td><td>".$row->dat_date."</td>";
								// echo "<td><button type='submit' class='btn btn-danger' value='".$row->pk_int_reg_id."' >Purchase Details</button></td>";
								echo "<td>".$row->pk_int_reg_id."</td><td>".$row->vchr_fname." </td><td>".$row->vchr_lname."</td><td>".$row->dat_date."</td>";
								echo "<td><button type='submit' class='btn btn-danger' value='".$row->fk_int_login_id."'  onclick='purchasedetails(this.value)'>Purchase Details</button></td>";
                    ...
                    <div class="col-sm-offset-1 col-sm-10" id="div2">
            				<div class="panel panel-default">
                				<div class="panel-heading" align="center">
